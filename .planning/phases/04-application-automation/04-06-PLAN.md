...
wave: 2
autonomous: true
objective: "Write 13 pytest-asyncio tests covering all five requirements (REQ-07 through REQ-11): unit tests for extract_resume_text and tailor_resume (mocked AsyncAnthropic), integration tests for POST /api/v1/applications/ and GET /api/v1/applications/, and task tests for status transitions and error reset. Target ≥85% coverage on the applications/ package."
...
# Plan 04-06: Integration Test Suite

## Objective

Create `tests/test_applications.py` with 13 tests that verify the complete Phase 4 application
lifecycle. Uses the same connection-level transaction rollback pattern from `tests/conftest.py`
(the `db` and `client` fixtures) to ensure test isolation without persistent DB state.

Dependencies: Plan 04-05 (all endpoints and tasks exist and are registered). This is Wave 2.

**Mocking rules (critical):**
- Mock `AsyncAnthropic` at its import site in the service module:
  `"jobplatform.applications.service.AsyncAnthropic"` — NOT `"anthropic.AsyncAnthropic"`
- Mock `tailor_and_apply.delay` (the Celery dispatch method) to prevent real broker calls
- For task integration tests, patch `"jobplatform.applications.tasks.AsyncSessionLocal"` to inject
  the test DB session (same `_inject_session` helper pattern as `tests/test_matching.py`)
- For task integration tests, patch `"jobplatform.applications.service.AsyncAnthropic"` to return
  a mock response

**Auth pattern:** Use `client.post("/api/v1/auth/register", ...)` + `client.post("/api/v1/auth/login", ...)` then pass `Authorization: Bearer <token>` header — same pattern as `test_matching.py` endpoint tests.

## Must Haves

- [ ] All 13 tests exist and pass: `uv run pytest tests/test_applications.py -q --tb=short` exits 0
- [ ] No test makes a real call to the Anthropic API
- [ ] No test dispatches a real Celery task (tailor_and_apply.delay is mocked)
- [ ] `test_create_application_success` verifies HTTP 202 + Application row in DB
- [ ] `test_duplicate_application_409` verifies second POST to same job_id returns 409
- [ ] `test_create_application_job_not_found` verifies nonexistent job_id returns 404
- [ ] `test_tailor_and_apply_status_transitions` verifies pending → tailoring → applied in DB
- [ ] `test_tailor_and_apply_error_resets_to_pending` verifies error branch resets status to pending
- [ ] `uv run pytest tests/ -q --tb=short` — full suite (including prior phases) still passes
- [ ] `uv run pytest tests/test_applications.py --cov=src/jobplatform/applications --cov-report=term-missing -q` shows ≥85% coverage

## Tasks

### Task 1: Create tests/test_applications.py with all 13 tests

**Files:**
- `tests/test_applications.py` — complete test suite for Phase 4

**Instructions:**

Create `tests/test_applications.py` with the exact contents below. The file uses
`pytest-asyncio` with `asyncio_mode = "auto"` (already set in `pyproject.toml`), the `db` and
`client` fixtures from `conftest.py`, and `unittest.mock` for external API mocking.
...
**Verify:**
- `uv run pytest tests/test_applications.py -q --tb=short` exits 0 with all 13 tests passing.
- `uv run pytest tests/test_applications.py --cov=src/jobplatform/applications --cov-report=term-missing -q` shows ≥85% coverage for the applications package.

## Self-Check

- [ ] `uv run pytest tests/test_applications.py -v --tb=short` lists exactly 13 tests, all PASSED
- [ ] No test output contains `ANTHROPIC_API_KEY` warnings or real API calls
- [ ] `uv run pytest tests/ -q --tb=short` — full test suite (all phases) passes
- [ ] Coverage for `src/jobplatform/applications/` is ≥85% as shown by `--cov` report
